...
def _draw_bboxes(frame_bgr: np.ndarray, data: np.lib.npyio.NpzFile, segment_idx: int, width: int, height: int, thickness: int) -> None:
    if "yolo_bboxes" not in data.files or "yolo_bbox_valid" not in data.files:
        return
    boxes = data["yolo_bboxes"]
    valid = data["yolo_bbox_valid"]
    if segment_idx >= boxes.shape[0] or boxes.shape[1] < 1 or boxes.shape[2] < 2:
        return
    for cls, group in enumerate(("left", "right")):
        if not bool(valid[segment_idx, 0, cls]):
            continue
        box = boxes[segment_idx, 0, cls].astype(np.float32)
        if not np.isfinite(box).all():
            continue
        x1, y1, x2, y2 = box.tolist()
        x1 = int(round(max(0.0, min(float(width - 1), x1))))
        x2 = int(round(max(0.0, min(float(width - 1), x2))))
        y1 = int(round(max(0.0, min(float(height - 1), y1))))
        y2 = int(round(max(0.0, min(float(height - 1), y2))))
        if x2 <= x1 or y2 <= y1:
            continue
        cv2.rectangle(frame_bgr, (x1, y1), (x2, y2), GROUP_COLORS_BGR[group], thickness, cv2.LINE_AA)
        ...
        try:
            for s in range(start_segment, end_segment):
                ...
                for t in range(seg_len):
                    ...
                    if args.draw_bboxes:
                        _draw_bboxes(frame, data, s, width, height, args.bbox_thickness)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--no-bboxes", dest="draw_bboxes", action="store_false", help="Do not draw YOLO left/right bboxes.")
    parser.set_defaults(draw_bboxes=True)
