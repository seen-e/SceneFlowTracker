...
class PipelineRunner:
    def __init__(self, cfg: dict[str, Any]) -> None:
        ...
        self.yolo_model: YoloModel | None = None
        self.cotracker_model: CoTrackerModel | None = None
    ...
    def _get_yolo(self) -> YoloModel:
        if self.yolo_model is None:
            self.yolo_model = YoloModel(self.cfg)
        return self.yolo_model

    def _get_cotracker(self) -> CoTrackerModel:
        if self.cotracker_model is None:
            self.cotracker_model = CoTrackerModel(self.cfg)
    ...
    def _process_yolo(self, items: list[FirstFrameItem]) -> list[YoloDetectionResult]:
        ...
        for batch in _chunks(items, int(ycfg.get("batch_size", 32))):
            batch_results, elapsed = model.predict_batch(batch)
            for result in batch_results:
                result.timings["yolo_batch_size"] = float(len(batch))
                result.timings["yolo_batch_fill_ratio"] = float(len(batch) / max(1, int(ycfg.get("batch_size", 32))))
    ...
    def _process_cotracker(self, items: list[DecodedTrackItem]) -> list:
        ...
        for item in sorted(items, key=lambda x: (x.job.frame_count != normal_t, x.job.episode_index, x.job.segment_id)):
            ...
            if item.job.frame_count != normal_t:
                ...
                results, _elapsed, _peak = model.track_batch(batch)
                track_results.extend(results)
            else:
