...
from .edges import bbox_mask, edge_map, filter_small_components, valid_image_mask
...
def sample_environment_points(image_rgb: np.ndarray, robot_bboxes: list[np.ndarray], target: int, cfg: dict, seed: int = 0, exclude_points: np.ndarray | None = None) -> dict:
    h, w = image_rgb.shape[:2]
    if target <= 0:
        return {"points_xy": np.empty((0, 2), np.float32), "features": {}, "stats": {"target_points": int(target), "final_sampled_count": 0}}
    valid = valid_image_mask(image_rgb)
    ...
    if len(picked) < target:
        fallback = _grid_points(valid, target * 4)
    ...
    return {
        "points_xy": picked,
        "features": features,
        "stats": {"target_points": int(target), "final_sampled_count": int(len(picked)), "candidate_count": int(len(points))},
    }
