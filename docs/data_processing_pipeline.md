...
```text
prepare batch -> model inference
```

也就是说，框架先按 `batch_size` 或 `segment_batch_size` 准备 batch，再把 batch 分配给不同模型 worker 推理。多 worker 只改变吞吐和显存占用，不改变采样点、bbox、CoTracker 输入或过滤规则。
...
- `segment_batch_size` 控制每个 CoTracker worker 一次推理几个视频片段。
